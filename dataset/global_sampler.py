...
class DistributedMiningSampler(DistributedSampler[T_co]):
    def __init__(self, dataset: Dataset, num_replicas: Optional[int] = None,
        ...
        print(self.queue_counter.shape)
    ...
    def load(self, path):
        ...
        # target = np.load(os.path.join(path,'queue.npy'))
        # # 调整数据形状
        # if self.queue.shape != target.shape:
        #     # 调整形状为一个相近的大小，可能并不完全符合目标形状
        #     self.queue = self.queue.reshape((-1,) + target[1:])
        #     print(self.queue.shape)  # 打印调整后的形状
        # else:
        #     self.queue = np.load(os.path.join(path,'queue.npy'))
        #     print(self.queue.shape)  # 打印调整后的形状
