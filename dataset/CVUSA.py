...
class CVUSA(torch.utils.data.Dataset):
    def __init__(self, mode = '', root = '/data1/lingxingtao/Dataset/CVUSA/', same_area=True, print_bool=False, polar = '',args=None): #CV-dataset
        ...
        self.sat_size = [112, 616]
        self.sat_size_default = [112, 616]

        self.grd_size = [112, 616]

        if args.sat_res != 0:
            # self.sat_size = [args.sat_res, args.sat_res]
            self.sat_size = [140, 770] # 112*12.25, 616*1.25
        ...
        with open(self.train_list, 'r') as file:
            ...
            for line in file:
                # data = line.split(',')
                data = line.replace('bingmap', 'polarmap').split(',')
